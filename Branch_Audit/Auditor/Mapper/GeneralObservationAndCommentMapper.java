...
public interface GeneralObservationAndCommentMapper {
        ...
        @Select("insert into branch_financial_audit(auditor_id, branch_id, category,observation,recommendation,impact,finding_date, case_number, drafted_date, audit_type, ing) OUTPUT inserted.id values(#{auditor.id}, #{auditor.branch.id}, #{auditor.category},#{finding},#{recommendation}, #{impact},#{finding_date}, #{case_number}, CURRENT_TIMESTAMP, 'GeneralObservation', #{ing})")
        Long createGeneralObservationAndComment(BranchFinancialAudit abnormalBalance);

        @Select("insert into branch_financial_audit(auditor_id, branch_id, category,observation,recommendation,impact,finding_date, case_number, drafted_date, finding_detail, audit_type, ing) OUTPUT inserted.id values(#{auditor.id}, #{auditor.branch.id}, #{auditor.category},#{finding},#{recommendation}, #{impact},#{finding_date}, #{case_number}, CURRENT_TIMESTAMP, #{finding_detail}, 'GeneralObservation', #{ing})")
        ...
        @Update("update branch_financial_audit set ing = #{ing},  observation = #{finding}, recommendation = #{recommendation}, impact=#{impact}, finding_date = #{finding_date}, is_edited = 1 where id=#{id}")
        ...
        @Update("update branch_financial_audit set ing = #{ing}, observation = #{finding}, finding_detail=#{finding_detail}, recommendation = #{recommendation}, impact=#{impact}, finding_date = #{finding_date}, is_edited = 1 where id=#{id}")
