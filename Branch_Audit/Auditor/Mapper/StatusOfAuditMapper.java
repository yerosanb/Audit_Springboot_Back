...
public interface StatusOfAuditMapper {
	...
	@Select("insert into branch_financial_audit(case_number, category, branch_id, auditor_id, finding_date, save_template, status, is_edited, auditor_status, drafted_date, audit_type, ing) OUTPUT inserted.id values(#{case_number},#{category}, #{auditor.branch.id}, #{auditor.id}, #{finding_date}, #{save_template}, 1, 1, 0, CURRENT_TIMESTAMP, 'Audit Status', #{ing})")
	...
	@Update("update branch_financial_audit set ing = #{ing}, finding_date=#{finding_date} where id=#{id}")
