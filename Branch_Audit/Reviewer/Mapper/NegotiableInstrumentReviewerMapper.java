...
@Mapper
public interface NegotiableInstrumentReviewerMapper {

	// @Select("select bfa.* from branch_financial_audit bfa "
	// + " inner join branch br on bfa.branch_id = br.id and br.region_id =
	// #{region_id} "
	// + " inner join negotiable_instrument_branch nib on nib.branch_audit_id =
	// bfa.id"
	...
	@Select("select *  from branch_financial_audit where id in (select branch_audit_id from negotiable_instrument_branch) and reviewer_id = #{reviewer_id}  and  auditor_status = 1 and status = 1 and review_status = 1 and approve_status != 1 and regional_compiler_status != 1 and rectification_status != 1 order by reviewed_date DESC ")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
