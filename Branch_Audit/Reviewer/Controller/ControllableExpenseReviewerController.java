...
import com.afr.fms.Branch_Audit.Reviewer.Service.ControllableExpenseReviewerService;
...
@RestController
@RequestMapping("/api/branch_audit/reviewer/controllable_expense")
@PreAuthorize("hasRole('REVIEWER_BFA')")
public class ControllableExpenseReviewerController {

	@Autowired
	private ControllableExpenseReviewerService auditService;
	@Autowired
	...
	if (functionalitiesService.verifyPermission(request,
		"get_pending_controllable_expense_reviewer_bfa")) {		try {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getReviewed_Findings(@PathVariable Long reviewer_id,HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_reviewed_controllable_expense_reviewer_bfa")) {
		try {
