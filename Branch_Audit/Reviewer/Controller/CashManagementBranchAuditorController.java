...
import com.afr.fms.Branch_Audit.Reviewer.Service.CashManagementBranchAuditorService;
...
@RestController
@RequestMapping("/api/branch_audit/reviewer/cash_management")
@PreAuthorize("hasRole('REVIEWER_BFA')")
public class CashManagementBranchAuditorController {

	@Autowired
	private CashManagementBranchAuditorService auditService;
	@Autowired
	...
	if (functionalitiesService.verifyPermission(request,
		"get_pending_cash_management_reviewer_bfa")) {
		try {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getReviewed_Findings(@PathVariable Long reviewer_id, HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_reviewed_cash_management_reviewer_bfa")) {
		try {
