...
import com.afr.fms.Branch_Audit.Reviewer.Service.OperationalDiscrepancyBranchReviewerService;
...
@RestController
@RequestMapping("/api/branch_audit/regional_compiler/operational_discrepancy")
@PreAuthorize("hasRole('REVIEWER_BFA')")
public class OperationalDiscrepancyBranchReviewerController {

	@Autowired
	private OperationalDiscrepancyBranchReviewerService auditService;
	@Autowired
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@RequestBody User user, HttpServletRequest request) {
		...
		"get_pending_operational_discrepancy_reviewer_bfa")) {
		try {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getReviewed_Findings(@PathVariable Long reviewer_id, HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_reviewed_operational_discrepancy_reviewer_bfa")) {
		try {
