...
public class ObservationReportSqlProvider2 {

	public String getBranchObservationAuditReport(@Param("region_id") Long region_id, @Param("branch_id") Long branch_id,
		...
		if (single_filter_info.equalsIgnoreCase("branch")) {
			sql.SELECT("	bfa.id, " + "	bfa.case_number, " + "	b.name as branch_name, "
					+ "	dbo._StripHTML( CASE "
					+ "    WHEN r.name IN ('ROLE_APPROVER_BFA',  'ROLE_COMPILER_BFA') THEN COALESCE(cra.finding, cba.finding, bfa.observation) "
					+ "    WHEN r.name IN ('ROLE_REVIEWER_BFA', 'ROLE_REGIONALD_BFA') THEN COALESCE(cba.finding, bfa.observation) "
					+ "    WHEN r.name IN ('ROLE_AUDITOR_BFA', 'ROLE_BRANCHM_BFA') THEN bfa.observation " + "    ELSE NULL "
...
//					.JOIN("branch as b on bfa.branch_id = b.id  ").WHERE("1 = 1");
					.JOIN("branch as b on bfa.branch_id = b.id  ").WHERE("1 = 1");
			...
			if (audit_finding != null)
				sql.AND().WHERE("dbo._StripHTML(bfa.observation) LIKE '%' + #{audit_finding} + '%'");
  ...
  //======================================================================================
	...
	//========================================================================================
		...
		sql.AND().WHERE("bfa.audit_type = 'GeneralObservation'");
