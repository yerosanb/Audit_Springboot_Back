...
import com.afr.fms.Branch_Audit.BranchManager.Service.LongOutstandingItemBranchManagerService;
...
@RestController
@RequestMapping("/api/branch/branch_manager/long_outstanding_item")
@PreAuthorize("hasRole('BRANCHM_BFA')")
public class LongOutstandingItemManagerController {
	@Autowired
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@PathVariable Long branch_id,
		...
		"get_pending_long_outstanding_items_branch_manager_bfa")) {
		try {

			return new ResponseEntity<>(longOutstandingItemBranchManagerService.getPendingAudits(branch_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRectifiedAudis(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(longOutstandingItemBranchManagerService.getRectifiedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRespondedAudits(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(longOutstandingItemBranchManagerService.getRespondedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<HttpStatus> add_response(@RequestBody BranchFinancialAudit auditee_response,
		...
		try {
			...
			longOutstandingItemBranchManagerService.branchManagerGiveResponse(auditee_response);
			// }
...
System.out.print(auditee_response);
		try {
			...
			longOutstandingItemBranchManagerService.branchManagerGiveResponseSelcted(auditee_response);
			// }
			// recentActivity.setMessage(audite.getName() + " branch is created ");
			// user.setId(branch.getUser_id());
			// recentActivity.setUser(user);
