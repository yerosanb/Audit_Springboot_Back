...
import com.afr.fms.Branch_Audit.BranchManager.Service.CashPerformanceBranchManagerService;
...
@RestController
@RequestMapping("/api/branch/branch_manager/cash_performance")
@PreAuthorize("hasRole('BRANCHM_BFA')")
public class CashPerformanceBranchManagerController {
	@Autowired
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@PathVariable Long branch_id,
		...
		"get_pending_cash_performance_branch_manager_bfa")) {
		try {

			return new ResponseEntity<>(cashPerformanceBranchManagerService.getPendingAudits(branch_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRectifiedAudis(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(cashPerformanceBranchManagerService.getRectifiedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRespondedAudits(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(cashPerformanceBranchManagerService.getRespondedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<HttpStatus> add_response(@RequestBody BranchFinancialAudit auditee_response,
		...
		try {
			...
			cashPerformanceBranchManagerService.branchManagerGiveResponse(auditee_response);
			// }
	...
	public ResponseEntity<HttpStatus> giveRespnseSelected(@RequestBody List<BranchFinancialAudit> auditee_response,
		...
		try {
			...
			cashPerformanceBranchManagerService.branchManagerGiveResponseSelcted(auditee_response);
			// }
			// recentActivity.setMessage(audite.getName() + " branch is created ");
			// user.setId(branch.getUser_id());
			// recentActivity.setUser(user);
