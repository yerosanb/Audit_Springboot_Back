...
import com.afr.fms.Branch_Audit.BranchManager.Service.SuspenseAndStockBranchManagerService;
...
@RestController
@RequestMapping("/api/branch/branch_manager/suspense_account")
@PreAuthorize("hasRole('BRANCHM_BFA')")
public class SuspenseAccountBranchManagerController {
	@Autowired
	private SuspenseAndStockBranchManagerService suspenseAndStockBranchManagerService;
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@PathVariable Long branch_id,
		...
		"get_pending_suspense_account_branch_manager_bfa")) {
		try {

			return new ResponseEntity<>(suspenseAndStockBranchManagerService.getPendingAudits(branch_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRectifiedAudis(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(suspenseAndStockBranchManagerService.getRectifiedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRespondedAudits(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(suspenseAndStockBranchManagerService.getRespondedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<HttpStatus> add_response(@RequestBody BranchFinancialAudit auditee_response,
		...
		try {
			...
			suspenseAndStockBranchManagerService.branchManagerGiveResponse(auditee_response);
			// }
...
System.out.print(auditee_response);
		try {
			...
			suspenseAndStockBranchManagerService.branchManagerGiveResponseSelcted(auditee_response);
			// }
			// recentActivity.setMessage(audite.getName() + " branch is created ");
			// user.setId(branch.getUser_id());
			// recentActivity.setUser(user);
