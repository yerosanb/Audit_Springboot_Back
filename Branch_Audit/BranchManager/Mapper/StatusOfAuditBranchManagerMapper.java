...
@Mapper
public interface StatusOfAuditBranchManagerMapper {

    @Select("select *  from branch_financial_audit where id in (select branch_audit_id from audit_status_branch )  and branch_id=#{branch_id} and auditor_status = 1 and status = 1  and rectification_status !=1 order by passed_date DESC")
	@Results(value = {
