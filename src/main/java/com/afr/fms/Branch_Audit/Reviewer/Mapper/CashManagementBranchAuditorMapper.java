...
@Mapper
public interface CashManagementBranchAuditorMapper {

	// @Select("select * from branch_financial_audit where category = #{category}
	// and id in (select branch_audit_id from cash_management_branch) and
	...
	@Select("<script>"
			...
			+ "</when>"
			+ "</choose>"
			+ " inner join cash_management_branch nib on nib.branch_audit_id = bfa.id"
	...
	@Select("select * from branch_financial_audit where review_status = 1 and reviewer_id=#{reviewer_id} and id in (select branch_audit_id from cash_management_branch) and status = 1 and regional_compiler_status != 1  Order By reviewed_date DESC")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
	...
	@Results(value = {
			...
			// "com.afr.fms.Auditor.Mapper.BranchFinancialAuditMapper.getISMAuditees")),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
