...
import com.afr.fms.Branch_Audit.Reviewer.Service.AssetLiabilityReviewerService;
import com.afr.fms.Common.Service.FunctionalitiesService;
import com.afr.fms.Payload.endpoint.Endpoint;

@CrossOrigin(origins = Endpoint.URL, maxAge = 3600, allowCredentials = "true")
@RestController
@RequestMapping("/api/branch/reviewer/asset_liability")
...
public class AssetLiabilityReviewerController {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@RequestBody User user,
		...
		"get_pending_asset_liability_reviewer_bfa")) {
		try {
			return new ResponseEntity<>(assetLiabilityReviewerService.getPendingAudits(user), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getReviewedFindingsForReviewer(@PathVariable Long reviewer_id,
			HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_reviewed_asset_liability_reviewer_bfa")) {
		try {
			return new ResponseEntity<>(assetLiabilityReviewerService.getReviewedAudits(reviewer_id), HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
