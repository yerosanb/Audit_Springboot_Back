...
import com.afr.fms.Branch_Audit.Reviewer.Service.IncompleteAccountBranchReviewerService;
...
@RestController
@RequestMapping("/api/branch_audit/reviewer/incomplete_account")
@PreAuthorize("hasRole('REVIEWER_BFA')")
public class IncompleteAccountBranchReviewerController {

	@Autowired
	private IncompleteAccountBranchReviewerService auditService;
	@Autowired
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@RequestBody User user,HttpServletRequest request) {
		...
		"get_pending_incomplete_account_reviewer_bfa")) {
		try {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getReviewed_Findings(@PathVariable Long reviewer_id, HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_reviewed_incomplete_account_reviewer_bfa")) {
		try {
