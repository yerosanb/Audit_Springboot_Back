...
@Mapper
public interface DormantAccountReviewerMapper {
	// @Select("select * from branch_financial_audit where category = #{category}
	// and id in (select branch_audit_id from dormant_inactive_account_branch) and
	// review_status = 0 and auditor_status = 1 and status = 1 Order By
	// finding_date")

	// @Select("select bfa.* from branch_financial_audit bfa "
	// + " inner join branch br on bfa.branch_id = br.id and br.region_id =
	// #{region_id} "
	// + " inner join dormant_inactive_account_branch nib on nib.branch_audit_id =
	// bfa.id"
	...
	@Select("<script>"
			...
			+ "</when>"
			+ "</choose>"
			+ " inner join dormant_inactive_account_branch nib on nib.branch_audit_id = bfa.id"
	...
	@Select("select * from branch_financial_audit where review_status = 1 and reviewer_id=#{reviewer_id} and id in (select branch_audit_id from dormant_inactive_account_branch) and status = 1 and  regional_compiler_status != 1 and rectification_status != 1  order By reviewed_date DESC")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
	...
	@Select("select * from branch_financial_audit where review_status = 1 and reviewer_id=#{reviewer_id} and status = 1 and approve_status = 1  and rectification_status != 1  Order By finding_date")
	@Results(value = {
			@Result(property = "id", column = "id"),
