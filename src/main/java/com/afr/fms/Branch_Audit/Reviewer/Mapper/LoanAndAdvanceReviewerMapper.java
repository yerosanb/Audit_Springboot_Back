...
@Mapper
public interface LoanAndAdvanceReviewerMapper {

        // @Select("select bfa.* from branch_financial_audit bfa "
        // + " inner join branch br on bfa.branch_id = br.id and br.region_id =
        // #{region_id} "
        // + " inner join loan_advance_branch nib on nib.branch_audit_id = bfa.id"
        ...
        @Select("select * from branch_financial_audit where id in (select branch_audit_id from loan_advance_branch) and review_status = 1 and regional_compiler_status != 1 and reviewer_id=#{reviewer_id} and status = 1 and rectification_status != 1 order By reviewed_date DESC")
        @Results(value = {
                        @Result(property = "id", column = "id"),
                        @Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
        ...
        public List<BranchFinancialAudit> getReviewedLoanAndAdvance(Long reviewer_id);
