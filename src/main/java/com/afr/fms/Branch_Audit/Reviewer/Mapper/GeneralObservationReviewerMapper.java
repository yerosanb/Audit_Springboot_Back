...
public interface GeneralObservationReviewerMapper {
        ...
        @Select("<script>"
                        ...
                        + "</when>"
                        + "</choose>"
                        + " where  bfa.auditor_status = 1 and bfa.status = 1 and bfa.banking = #{auditType} and (bfa.observation is not null) and bfa.rectification_status != 1 and bfa.review_status != 1 and bfa.regional_compiler_status != 1 ORDER BY bfa.passed_date DESC"
                        + "</script>")
