...
public interface LongOutstandingItemsReviewerMapper {
	...
	// #{region_id}"
	// + " inner join long_outstanding_item_branch nib on nib.branch_audit_id =
	// bfa.id"
	...
	@Select("<script>"
			...
			+ "</when>"
			+ "</choose>"
			+ " inner join long_outstanding_item_branch nib on nib.branch_audit_id = bfa.id"
	...
	public List<BranchFinancialAudit> getLongOutstandingItemsForReviewer(@Param("auditType") String auditType,
			User user);

	@Select("select * from branch_financial_audit where id in (select branch_audit_id from long_outstanding_item_branch) and review_status = 1 and regional_compiler_status != 1 and reviewer_id=#{reviewer_id} and status = 1 and rectification_status != 1 order By reviewed_date DESC ")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
