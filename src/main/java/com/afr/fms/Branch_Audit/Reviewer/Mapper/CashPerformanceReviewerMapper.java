...
@Mapper
public interface CashPerformanceReviewerMapper {
        ...
        @Select("<script>"
                        ...
                        + "</when>"
                        + "</choose>"
                        + " inner join cash_performance_branch nib on nib.branch_audit_id = bfa.id"
        ...
        public List<BranchFinancialAudit> getAuditsForBranchReviewer(@Param("auditType") String auditType, User user);

        @Select("select * from branch_financial_audit where review_status = 1 and reviewer_id=#{reviewer_id} and id in (select branch_audit_id from cash_performance_branch) and status = 1 and regional_compiler_status != 1  Order By reviewed_date DESC")
        @Results(value = {
                        @Result(property = "id", column = "id"),
                        @Result(property = "branch", column = "branch_id", one = @One(select = "com.afr.fms.Admin.Mapper.BranchMapper.getBranchById")),
        ...
        public List<BranchFinancialAudit> getReviewed_cash_Findings(Long reviewer_id);
