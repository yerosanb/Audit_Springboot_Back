...
import com.afr.fms.Branch_Audit.BranchManager.Service.IncompleteAccountBranchManagerService;
...
@RestController
@RequestMapping("/api/branch/branch_manager/incomplete_account")
@PreAuthorize("hasRole('BRANCHM_BFA')")
public class IncompleteAccountBranchManagerController {
	@Autowired
	private IncompleteAccountBranchManagerService incompleteAccountBranchManagerService;
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@PathVariable Long branch_id,
		...
		"get_pending_incomplete_account_branch_manager_bfa")) {
		try {

			return new ResponseEntity<>(incompleteAccountBranchManagerService.getPendingAudits(branch_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRectifiedAudis(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(incompleteAccountBranchManagerService.getRectifiedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRespondedAudits(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(incompleteAccountBranchManagerService.getRespondedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<HttpStatus> add_response(@RequestBody BranchFinancialAudit auditee_response,
		...
		try {
			...
			incompleteAccountBranchManagerService.branchManagerGiveResponse(auditee_response);
			// }
	...
	public ResponseEntity<HttpStatus> giveRespnseSelected(@RequestBody List<BranchFinancialAudit> auditee_response,
		...
		try {
			...
			incompleteAccountBranchManagerService.branchManagerGiveResponseSelcted(auditee_response);
			// }
			// recentActivity.setMessage(audite.getName() + " branch is created ");
			// user.setId(branch.getUser_id());
			// recentActivity.setUser(user);
