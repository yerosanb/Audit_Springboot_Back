...
@Mapper
public interface LongOutstandingItemBranchManagerMapper {

	@Select("select *  from branch_financial_audit where id in (select branch_audit_id from long_outstanding_item_branch )  and branch_id=#{branch_id} and auditor_status = 1 and status = 1  and rectification_status !=1 and response_added !=1 order by passed_date DESC ")
	@Results(value = {
	...
	@Select("select *  from branch_financial_audit where id in (select branch_audit_id from abnormal_balance_branch )  and auditee_id=#{auditee_id} and auditor_status = 1 and status = 1 and rectification_status = 1 and response_added !=1 order by reviewed_date DESC ")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
			@Result(property = "reviewer", column = "reviewer_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
	...
	@Select("select *  from branch_financial_audit where id in (select branch_audit_id from abnormal_balance_branch )  and auditee_id=#{auditee_id} and auditor_status = 1  and rectification_status=1 and status = 1 and response_added = 1 ")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
			@Result(property = "reviewer", column = "reviewer_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
