...
@Mapper
public interface GeneralObservationBranchManagerMapper {

	@Select("select *  from branch_financial_audit where  branch_id=#{branch_id} and auditor_status = 1 and status = 1  and rectification_status !=1 and observation is not null and response_added !=1 order by passed_date DESC ")
	@Results(value = {
	...
	@Select("select *  from branch_financial_audit where auditee_id=#{auditee_id} and auditor_status = 1 and status = 1 and rectification_status = 1 and response_added !=1")
	@Results(value = {
			@Result(property = "id", column = "id"),
			@Result(property = "auditor", column = "auditor_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
			@Result(property = "reviewer", column = "reviewer_id", one = @One(select = "com.afr.fms.Admin.Mapper.UserMapper.getAuditorById")),
