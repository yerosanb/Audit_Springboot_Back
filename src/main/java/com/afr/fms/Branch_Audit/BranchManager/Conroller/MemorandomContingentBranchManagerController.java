...
import com.afr.fms.Branch_Audit.BranchManager.Service.MemorandomContiongentBranchManagerService;
...
@RestController
@RequestMapping("/api/branch/branch_manager/memorandom-contingent")
@PreAuthorize("hasRole('BRANCHM_BFA')")
public class MemorandomContingentBranchManagerController {
	@Autowired
	private MemorandomContiongentBranchManagerService memorandomContiongentBranchManagerService;
	...
	public ResponseEntity<List<BranchFinancialAudit>> getPendingAudits(@PathVariable Long branch_id,
		...
		"get_pending_memorandom_contingent_branch_manager_bfa")) {
		try {

			return new ResponseEntity<>(memorandomContiongentBranchManagerService.getPendingAudits(branch_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRectifiedAudis(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(memorandomContiongentBranchManagerService.getRectifiedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<List<BranchFinancialAudit>> getRespondedAudits(@PathVariable Long auditee_id,
		...
		try {
			...
			return new ResponseEntity<>(memorandomContiongentBranchManagerService.getRespondedAudits(auditee_id),
					HttpStatus.OK);
		} catch (Exception ex) {
			System.out.println(ex);
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<HttpStatus> add_response(@RequestBody BranchFinancialAudit auditee_response,
		...
		try {
			...
			memorandomContiongentBranchManagerService.branchManagerGiveResponse(auditee_response);
			// }
	...
	public ResponseEntity<HttpStatus> giveRespnseSelected(@RequestBody List<BranchFinancialAudit> auditee_response,
		...
		// if (functionalitiesService.verifyPermission(request,
		// "create_or_update_ISM_auditee_response")) {
System.out.print(auditee_response);
		try {

			memorandomContiongentBranchManagerService.branchManagerGiveResponseSelcted(auditee_response);
			// }
			// recentActivity.setMessage(audite.getName() + " branch is created ");
			// user.setId(branch.getUser_id());
			// recentActivity.setUser(user);
