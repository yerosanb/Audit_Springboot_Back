...
@RestController
@RequestMapping("/api/remark_compiled_branch_audit")
public class RemarkCompiledBranchAuditController {

    @Autowired
    RemarkCompiledBranchAuditService remarkService;
    ...
    private ResponseEntity<List<RemarkBranchAudit>> getRemarks(@RequestBody RemarkBranchAudit remark, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "get_remark_compiled_branch_bfa")) {
    ...
    private ResponseEntity<List<User>> getUsersByCategory(@PathVariable String category, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "get_user_remark_compiled_branch_bfa")) {
        try {
    ...
    private ResponseEntity<List<RemarkBranchAudit>> getUnseenRemarks(@RequestBody RemarkBranchAudit remark, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "get_unseen_remark_compiled_branch_bfa")) {
            try {
    ...
    private ResponseEntity<HttpStatus> seenRemark(@RequestBody RemarkBranchAudit remark, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "get_seen_remark_compiled_branch_bfa")) {
        try {
    ...
    private ResponseEntity<HttpStatus> setReciever(@RequestBody List<RemarkBranchAudit> remarks, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "set_reciever_remark_compiled_branch_bfa")) {
        try {
    ...
    private ResponseEntity<HttpStatus> addRemark(@RequestBody RemarkBranchAudit remark, HttpServletRequest request) {
        if (functionalitiesService.verifyPermission(request, "add_remarks_remark_compiled_branch_bfa")) {
