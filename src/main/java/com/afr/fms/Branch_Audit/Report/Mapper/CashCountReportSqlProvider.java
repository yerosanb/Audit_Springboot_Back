...
public class CashCountReportSqlProvider {

	public String getBranchCashCountAuditReport(@Param("region_id") Long region_id, @Param("branch_id") Long branch_id,
		...
		if (!single_filter_info.equalsIgnoreCase("general"))
			if (!regional_compiler_compiled_date[0].equalsIgnoreCase("none42e") ||
					...
					cash_count_type != null || banking != null)
		...
		if (single_filter_info.equalsIgnoreCase("branch")) {
			sql.SELECT("	bfa.id, " + "	bfa.case_number, " + "	b.name as branch_name, "
					...
					+ "	acb.cash_count_type as cash_count_type, "
					+ "	acb.actual_cash_count as actual_cash_count, "
					+ "	acb.trial_balance as trial_balance, "
					+ "	acb.difference as difference, "
					+ "	acb.atm_amount_branch as atm_amount_branch, "
 					+ "	acb.atm_amount_lobby as atm_amount_lobby, "
 					+ "	acb.case_number as case_number, "
					...
					.JOIN("cash_count_branch AS acb ON bfa.id = acb.branch_audit_id")
			...
			sql.SELECT("	bfa.id, " + "	re.name as region_name, " + "	dbo._StripHTML( CASE "
					...
					+ "    WHEN r.name IN ('ROLE_REVIEWER_BFA', 'ROLE_REGIONALD_BFA') THEN COALESCE(cba.finding, bfa.finding) "
					+ "    WHEN r.name IN ('ROLE_AUDITOR_BFA', 'ROLE_BRANCHM_BFA') THEN bfa.finding " + "    ELSE NULL "
					+ "  END) AS audit_finding, " + "	acb.cash_count_type as cash_count_type, "
					+ "	acb.actual_cash_count as actual_cash_count, "
					+ "	acb.trial_balance as trial_balance, "
					+ "	acb.difference as difference, "
					+ "	acb.atm_amount_branch as atm_amount_branch, "
					+ "	acb.case_number as case_number, "
 					+ "	acb.atm_amount_lobby as atm_amount_lobby,  " + "	dbo._StripHTML( CASE "
					+ "    WHEN r.name IN ('ROLE_APPROVER_BFA',  'ROLE_COMPILER_BFA') THEN COALESCE(cra.impact, cba.impact, bfa.impact) "
					+ "    WHEN r.name IN ('ROLE_REVIEWER_BFA', 'ROLE_REGIONALD_BFA') THEN COALESCE(cba.impact, bfa.impact) "
					+ "    WHEN r.name IN ('ROLE_AUDITOR_BFA', 'ROLE_BRANCHM_BFA') THEN bfa.impact " + "    ELSE NULL "
					+ "  END) AS audit_impact, " + "	dbo._StripHTML( CASE "
			...
			sql.SELECT("	bfa.id, " + "	re.name as region_name, " + "	dbo._StripHTML( CASE "
					...
					.JOIN("cash_count_branch AS acb ON bfa.id = acb.branch_audit_id")
					.JOIN("user_role AS ur ON ur.user_id = #{user_id}").JOIN("role AS r ON ur.role_id = r.id")
					//.JOIN("operational_descripancy_category AS odc ON acb.operational_descripancy_category_id = odc.id")
...
//				sql.AND().WHERE("b.region_id = #{region_id}");
			...
			if (cash_count_min != null)
				sql.AND().WHERE("acb.actual_cash_count >= #{cash_count_min} ");

			if (cash_count_max != null)
				sql.AND().WHERE("acb.actual_cash_count <= #{cash_count_max} ");

			if (trial_balance_min != null)
				sql.AND().WHERE("acb.trial_balance >= #{trial_balance_min} ");

			if (trial_balance_max != null)
				sql.AND().WHERE("acb.trial_balance <= #{trial_balance_max} ");
	...
	//======================================================================================
			...
			if (cash_count_type != null)
				sql.AND().WHERE("acb.cash_count_type  LIKE '%' + #{cash_count_type} + '%'");
		}
