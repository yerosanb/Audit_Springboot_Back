...
public class NegotiableReportSqlProvider2 {

	public String getBranchNegotiableAuditReport(@Param("region_id") Long region_id, @Param("branch_id") Long branch_id,
			...
			@Param("account_number") String account_number, @Param("min_amount") Double min_amount,
			@Param("max_amount") Double max_amount, @Param("rectification_status") String rectification_status,
    		...
    		@Param("banking") String banking, @Param("stock_item_type") String stock_item_type,
		...
		if (!single_filter_info.equalsIgnoreCase("general"))
			if (!regional_compiler_compiled_date[0].equalsIgnoreCase("none42e") ||
				...
				account_number != null || banking != null || stock_item_type != null || cash_type != null)
		...
		if (single_filter_info.equalsIgnoreCase("branch")) {
			sql.SELECT("	bfa.id, " + "	bfa.case_number, " + "	b.name as branch_name, "
					...
					+ "	acb.account_number as account_number, "
					+ "	acb.account_holder as account_holder_name, "
					+ "	acb.printed_date as date_printed, "
					+ "	acb.ck_type as type_of_ck, "
					+ "	acb.ck_range as ck_range, "
					+ "	acb.quantity as quantity, "
					+ "	acb.unit_price as unit_price, "
					+ "	acb.amount as amount, "
					+ "	acb.trial_balance as trial_balance_amount, "
					+ "	acb.cash_type as cash_type, "
					+ "	acb.fcy as fcy, "
					+ "	nsi.stock_type as stock_type, "
					...
					.JOIN("negotiable_instrument_branch AS acb ON bfa.id = acb.branch_audit_id")
					.JOIN("negotiable_stock_item as nsi ON acb.negotiable_stock_item_id = nsi.id")
			...
			sql.SELECT("	bfa.id, " + "	re.name as region_name, " + "	dbo._StripHTML( CASE "
					...
					+ "	acb.fcy as fcy, "
					+ "	nsi.stock_type as stock_type, "
					+ "	acb.trial_balance as trial_balance_amount," + "	dbo._StripHTML( CASE "
					+ "    WHEN r.name IN ('ROLE_APPROVER_BFA',  'ROLE_COMPILER_BFA') THEN COALESCE(cra.impact, cba.impact, bfa.impact) "
					+ "    WHEN r.name IN ('ROLE_REVIEWER_BFA', 'ROLE_REGIONALD_BFA') THEN COALESCE(cba.impact, bfa.impact) "
					+ "    WHEN r.name IN ('ROLE_AUDITOR_BFA', 'ROLE_BRANCHM_BFA') THEN bfa.impact " + "    ELSE NULL "
					+ "  END) AS audit_impact, " + "	dbo._StripHTML( CASE "
			...
			if (max_amount != null)
				sql.AND().WHERE("acb.amount <= #{max_amount} ");

			if (min_amount != null)
				sql.AND().WHERE("acb.amount >= #{min_amount} ");
			...
			if (stock_item_type != null)
				sql.AND().WHERE("nsi.id = #{stock_item_type}");
			if (region_id != null)
				sql.AND().WHERE("b.region_id = #{region_id}");
			if (branch_id != null)
				sql.AND().WHERE("b.id = #{branch_id}");
			...
			// if (amount != null)
			// sql.AND().WHERE("acb.amount = #{amount}");
