...
public class SqlProviderDormantInactiveAccount {

	public String getBranchAuditReportDormantInactiveAccount(@Param("region_id") Long region_id,
			...
			@Param("account_type") String account_type, @Param("amount_min") Double amount_min,
			@Param("amount_max") Double amount_max, @Param("account_number") String account_number,
		...
		if (!single_filter_info.equalsIgnoreCase("general"))
			if (!regional_compiler_compiled_date[0].equalsIgnoreCase("none42e") ||
				...
				account_type != null || account_number != null || banking != null || cash_type != null)
		...
		if (single_filter_info.equalsIgnoreCase("branch")) {
			sql.SELECT("	bfa.id, " + "	bfa.case_number, " + "	b.name as branch_name, "
					...
					+ "	diab.account_number as account_number, " + "	diab.deactivated_date as deactivated_date, "
					+ "	diab.amount as amount, " + "	diab.entry_passed_by as entry_passed_by, "
					+ "	diab.entry_approved_by as entry_approved_by, " + "	diab.account_type as account_type, "
					+ "	diab.cash_type as cash_type, "
					+ "	diab.fcy as fcy, "
					+ "	diab.acount_name as account_name, "
					...
					.JOIN("dormant_inactive_account_branch AS diab ON bfa.id = diab.branch_audit_id")
					.JOIN("user_role AS ur ON ur.user_id = #{user_id}").JOIN("role AS r ON ur.role_id = r.id")
					.JOIN("branch as b on bfa.branch_id = b.id and bfa.id = diab.branch_audit_id").WHERE("1 = 1");
			if (branch_id != null)
...
//				sql.AND().WHERE("b.region_id = #{region_id}");
			...
			if (amount_min != null)
				sql.AND().WHERE("diab.amount >= #{amount_min} ");
			if (amount_max != null)
				sql.AND().WHERE("diab.amount <= #{amount_max} ");
  ...
  //======================================================================================
			...
			/*
			 *
			 * @Param("account_type") String account_type, @Param("amount_min") Long
			 * amount_min,
			 *
			 * @Param("amount_max") Long amount_max, @Param("account_number") String
			 * account_number,
			 */
			if (cash_type != null)
				sql.AND().WHERE("diab.cash_type = #{cash_type}");
			if (amount_min != null)
				sql.AND().WHERE("diab.amount >= #{amount_min} ");
			if (amount_max != null)
				sql.AND().WHERE("diab.amount <= #{amount_max} ");
			...
			if (account_type != null)
				sql.AND().WHERE("diab.account_type LIKE '%' + #{account_type} + '%'");
			if (account_number != null)
				sql.AND().WHERE("diab.account_number LIKE '%' + #{account_number} + '%'");
		}
