...
public class SqlProviderCashManagement {

	public String getBranchAuditReportCashManagement(@Param("region_id") Long region_id,
			...
			@Param("mid_rate_fcy_min") Double mid_rate_fcy_min, @Param("mid_rate_fcy_max") Double mid_rate_fcy_max,
			@Param("average_cash_holding_min") Double average_cash_holding_min,
			@Param("average_cash_holding_max") Double average_cash_holding_max, @Param("cash_type") String cash_type,
		...
		System.out.println("mid_rate_fcy_min: " + mid_rate_fcy_min);
		System.out.println("mid_rate_fcy_max: " + mid_rate_fcy_max);
		System.out.println("average_cash_holding_min: " + average_cash_holding_min);
		System.out.println("average_cash_holding_max: " + average_cash_holding_max);
		System.out.println("cash_type: " + cash_type);
		...
		if (single_filter_info.equalsIgnoreCase("branch")) {
			sql.SELECT("bfa.id, " + "bfa.case_number, " + "b.name as branch_name, " + "dbo._StripHTML( CASE "
					...
					+ "    amb.average_cash_holding AS average_cash_holding, "
					+ "    amb.branch_cash_set_limit AS branch_cash_set_limit, "
					+ "    amb.mid_rate_fcy AS mid_rate_fcy, " + "    amb.difference AS difference, "
					+ "	   amb.cash_type AS cash_type,"
					+ "	   amb.fcy AS fcy,"
					...
					.JOIN("cash_management_branch AS amb ON bfa.id = amb.branch_audit_id")
					.JOIN("user_role AS ur ON ur.user_id = #{user_id}").JOIN("role AS r ON ur.role_id = r.id")
					.JOIN("branch as b on bfa.branch_id = b.id and bfa.id = amb.branch_audit_id").WHERE("1 = 1");
			...
			if (mid_rate_fcy_min != null)
				sql.AND().WHERE("amb.mid_rate_fcy >= #{mid_rate_fcy_min} ");
			if (mid_rate_fcy_max != null)
				sql.AND().WHERE("amb.mid_rate_fcy <= #{mid_rate_fcy_max} ");
			if (average_cash_holding_min != null)
				sql.AND().WHERE("amb.average_cash_holding >= #{average_cash_holding_min} ");
			if (average_cash_holding_max != null)
				sql.AND().WHERE("amb.average_cash_holding <= #{average_cash_holding_max} ");
			...
			if (cash_type != null)
				sql.AND().WHERE("amb.cash_type = #{cash_type}");
			if (mid_rate_fcy_min != null)
				sql.AND().WHERE("amb.mid_rate_fcy >= #{mid_rate_fcy_min} ");
			if (mid_rate_fcy_max != null)
				sql.AND().WHERE("amb.mid_rate_fcy <= #{mid_rate_fcy_max} ");
