...
public interface DormantAccountMapper {
	@Select("insert into branch_financial_audit(case_number, branch_id, auditor_id, finding_date, finding, impact, recommendation, save_template,category, status, is_edited, auditor_status, audit_type, drafted_date, banking) OUTPUT inserted.id values(#{case_number}, #{auditor.branch.id}, #{auditor.id}, #{finding_date}, #{finding}, #{impact}, #{recommendation}, #{save_template},#{category}, 1, 1, 0, #{audit_type}, CURRENT_TIMESTAMP, #{banking})")
	public Long createBAFinding(BranchFinancialAudit audit);

	@Select("insert into branch_financial_audit(case_number, branch_id, auditor_id, finding_date, finding, impact, recommendation, save_template,category, status, is_edited, auditor_status, audit_type, finding_detail, drafted_date, banking) OUTPUT inserted.id values(#{case_number}, #{auditor.branch.id}, #{auditor.id}, #{finding_date}, #{finding}, #{impact}, #{recommendation}, #{save_template},#{category}, 1, 1, 0, #{audit_type}, #{finding_detail}, CURRENT_TIMESTAMP, #{banking})")
