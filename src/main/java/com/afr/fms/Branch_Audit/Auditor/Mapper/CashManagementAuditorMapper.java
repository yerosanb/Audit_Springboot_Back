...
public interface CashManagementAuditorMapper {
	@Select("insert into branch_financial_audit(case_number, auditor_id,branch_id, finding_date, finding, audit_type, impact, recommendation, save_template,category, status, is_edited, auditor_status,drafted_date, banking) OUTPUT inserted.id values(#{case_number}, #{auditor.id}, #{auditor.branch.id}, #{finding_date}, #{finding},'cash_management_branch', #{impact}, #{recommendation}, #{save_template},#{category}, 1, 1, 0,CURRENT_TIMESTAMP, #{banking})")
	...
	@Select("insert into branch_financial_audit(case_number, auditor_id,branch_id, finding_date, finding, finding_detail, audit_type, impact, recommendation, save_template,category, status, is_edited, auditor_status,drafted_date, banking) OUTPUT inserted.id values(#{case_number}, #{auditor.id}, #{auditor.branch.id}, #{finding_date}, #{finding},#{finding_detail},'cash_management_branch', #{impact}, #{recommendation}, #{save_template},#{category}, 1, 1, 0,CURRENT_TIMESTAMP, #{banking})")
