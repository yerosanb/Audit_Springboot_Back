...
public interface AtmCardAuditorMapper {
        ...
        @Select("insert into branch_financial_audit(case_number, finding, audit_type, impact, recommendation,finding_date,auditor_id,category, status, drafted_date, is_edited, branch_id, banking) OUTPUT inserted.id values(#{case_number},#{finding},'ATMCard', #{impact}, #{recommendation}, #{finding_date}, #{auditor.id}, #{category}, 1, CURRENT_TIMESTAMP, 1, #{auditor.branch.id}, #{banking})")
        ...
        @Select("insert into branch_financial_audit(case_number, finding, finding_detail, audit_type, impact, recommendation,finding_date,auditor_id,category, status, is_edited, branch_id, banking) OUTPUT inserted.id values(#{case_number},#{finding},#{finding_detail},'ATMCard', #{impact}, #{recommendation}, #{finding_date}, #{auditor.id}, #{category}, 1, 1, #{auditor.branch.id}, #{banking})")
