...
public interface LoanAndAdvanceMapper {
        ...
        @Select("insert into branch_financial_audit(case_number,category, auditor_id, finding, impact, recommendation, status, is_edited, auditor_status, drafted_date, branch_id, audit_type, banking) OUTPUT inserted.id values(#{case_number},#{auditor.category}, #{auditor.id}, #{finding}, #{impact}, #{recommendation}, 1, 1, 0, CURRENT_TIMESTAMP, #{auditor.branch.id}, #{audit_type}, #{banking})")
        public Long createBAFinding(BranchFinancialAudit audit);

        @Select("insert into branch_financial_audit(case_number,category, auditor_id, finding, impact, recommendation, status, is_edited, auditor_status, drafted_date, branch_id, finding_detail, audit_type, banking) OUTPUT inserted.id values(#{case_number},#{auditor.category}, #{auditor.id}, #{finding}, #{impact}, #{recommendation}, 1, 1, 0, CURRENT_TIMESTAMP, #{auditor.branch.id}, #{finding_detail}, #{audit_type}, #{banking})")
        ...
        @Update("update branch_financial_audit set banking = #{banking}, finding=#{finding}, impact=#{impact}, recommendation=#{recommendation}, is_edited = 1 where id=#{id}")
        public void updateBFA(BranchFinancialAudit audit);
        
        @Update("update branch_financial_audit set banking = #{banking}, finding=#{finding}, impact=#{impact}, recommendation=#{recommendation}, is_edited = 1, finding_detail = #{finding_detail} where id=#{id}")
