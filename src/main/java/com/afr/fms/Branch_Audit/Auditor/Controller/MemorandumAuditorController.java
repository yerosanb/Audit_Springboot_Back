...
import com.afr.fms.Branch_Audit.Auditor.Service.MemorandumAuditorService;
...
@RestController
@RequestMapping("/api/branch_audit/auditor/memorandum_contigent")
@PreAuthorize("hasRole('AUDITOR_BFA')")
public class MemorandumAuditorController {

	@Autowired
	private MemorandumAuditorService auditService;
	@Autowired
	...
	public ResponseEntity<List<BranchFinancialAudit>> getBAFindingsForAuditor(@PathVariable Long auditor_id,
		...
		"get_passed_memorandum_auditor_bfa")) {
		try {
			return new ResponseEntity<>(auditService.getPassedAuditsForAuditor(auditor_id), HttpStatus.OK);
		} catch (Exception ex) {
	...
	public ResponseEntity<List<BranchFinancialAudit>> getAuditsOnDrafting(@PathVariable Long auditor_id,
			HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"get_drafted_memorandum_auditor_bfa")) {
		try {
	...
	public ResponseEntity<HttpStatus> createBAFinding(@RequestBody BranchFinancialAudit audit,
			HttpServletRequest request) {
		if (functionalitiesService.verifyPermission(request,
		"create_or_update_memorandum_auditor_bfa")) {
		try {
			if (audit.getId() != null) {
