...
public interface IncompleteAccountBranchMapper {
	@Select("insert into branch_financial_audit(case_number, category, auditor_id, finding_date, finding, impact, recommendation, save_template, status, is_edited, auditor_status, drafted_date, branch_id, audit_type, banking) OUTPUT inserted.id values(#{case_number}, #{category}, #{auditor.id}, #{finding_date}, #{finding}, #{impact}, #{recommendation}, #{save_template}, 1, 1, 0, CURRENT_TIMESTAMP,#{auditor.branch.id}, #{audit_type}, #{banking})")
	public Long createBAFinding(BranchFinancialAudit audit);

	@Select("insert into branch_financial_audit(case_number, category, auditor_id, finding_date, finding, finding_detail, impact, recommendation, save_template, status, is_edited, auditor_status, drafted_date, branch_id, audit_type, banking) OUTPUT inserted.id values(#{case_number}, #{category}, #{auditor.id}, #{finding_date}, #{finding}, #{finding_detail}, #{impact}, #{recommendation}, #{save_template}, 1, 1, 0, CURRENT_TIMESTAMP,#{auditor.branch.id}, #{audit_type}, #{banking})")
