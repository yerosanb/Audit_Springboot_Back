...
public interface LongOutStandingItemsAuditorMapper {
	...
	@Select("insert into branch_financial_audit(case_number, category, auditor_id, finding_date, finding, finding_detail, audit_type, impact, recommendation, save_template, status, is_edited, auditor_status, drafted_date, branch_id, banking) OUTPUT inserted.id values(#{case_number},#{category}, #{auditor.id}, #{finding_date}, #{finding},#{finding_detail},'long_outstanding_items_branch', #{impact}, #{recommendation}, #{save_template}, 1, 1, 0, CURRENT_TIMESTAMP, #{auditor.branch.id}, #{banking})")
	public Long insertIntoBranchFinancialWithFindingDetail(BranchFinancialAudit audit);

	@Select("insert into branch_financial_audit(case_number, category, auditor_id, finding_date, finding, audit_type, impact, recommendation, save_template, status, is_edited, auditor_status, drafted_date, branch_id, banking) OUTPUT inserted.id values(#{case_number},#{category}, #{auditor.id}, #{finding_date}, #{finding},'long_outstanding_items_branch', #{impact}, #{recommendation}, #{save_template}, 1, 1, 0, CURRENT_TIMESTAMP, #{auditor.branch.id}, #{banking})")
